...
CREATE VIEW product_name AS SELECT products.name AS product, catalogs.name AS catalog_name FROM products, catalogs
LEFT JOIN catalogs ON products.catalog_id = catalogs.id;

SELECT * FROM product_name;

-- II. Хранимые процедуры
...
BEGIN
  RETURN CASE
      ...
      WHEN "18:00" <= CURTIME() AND CURTIME() < "21:00" THEN "Добрый вечер"
      ELSE "Доброй ночи"
    END;
END //
...
BEGIN
  IF NEW.name IS NULL AND NEW.description IS NULL
    THEN SIGNAL sqlstate '45001' set message_text = "products name and description can not be NULL"; 
  end if;
END; //
