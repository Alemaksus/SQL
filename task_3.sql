...
-- new tables for task_3:

DROP TABLE IF EXISTS friends;
CREATE TABLE my_friends (
	id SERIAL PRIMARY KEY,
	name_friend VARCHAR(100) COMMENT 'Имя друга',
    lastname_friend VARCHAR(100) COMMENT 'Фамилия друга',	
	    
    FOREIGN KEY (approved_friend_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE,
    -- FOREIGN KEY (to_user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE
   
DROP TABLE IF EXISTS news_feed;
CREATE TABLE news_feed (
	id SERIAL PRIMARY KEY,
	my_news BIGINT UNSIGNED COMMENT 'Новости от меня',
	friends_news BIGINT UNSIGNED COMMENT 'Новости от друзей',
    communities_news BIGINT UNSIGNED COMMENT 'Новости сообществ',
    body TEXT,
    created_at DATETIME DEFAULT NOW(),
    
    FOREIGN KEY (from_friends_id) REFERENCES friends(id) ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (from_community_name) REFERENCES communities_name_idx(name) ON UPDATE CASCADE ON DELETE CASCADE   
